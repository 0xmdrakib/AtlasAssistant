...
export function FloatingAudioPlayer({ player }: { player: SpeechPlayer }) {
  ...
  const playLabel = playing ? (bn ? "অডিও থামান" : "Pause audio") : state.status === "ended" ? (bn ? "আবার শুনুন" : "Replay audio") : (bn ? "অডিও চালান" : "Play audio");
  ...
  return (
    <div ref={region} className="floating-audio-player" role="region" aria-label={bn ? "অডিও প্লেয়ার" : "Audio player"} aria-describedby={captionId} data-audio-player data-playback={state.status}
      onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); close(); } }}>
      <div className="audio-player-caption"><span className="audio-player-dot" aria-hidden="true" /><span id={captionId} className="audio-player-title" title={state.track.title}>{state.track.title || (bn ? "সংবাদ শুনুন" : "News audio")}</span></div>
      <div className="audio-player-controls">
        ...
        <button type="button" className="audio-player-control audio-player-close" onClick={close} aria-label={bn ? "অডিও প্লেয়ার বন্ধ করুন" : "Close audio player"} title={bn ? "বন্ধ করুন ও শুরুতে ফিরুন" : "Close and reset"}><X size={27} strokeWidth={1.8} /></button>
      </div>
      <div className="audio-player-progress">
        <input type="range" min={0} max={Math.ceil(state.duration * 10) / 10} step={0.1} value={position}
          ...
          onKeyDown={(event) => {
            const steps: Record<string, number> = { ArrowLeft: -10, ArrowDown: -10, PageDown: -10, ArrowRight: 10, ArrowUp: 10, PageUp: 10 };
            if (!(event.key in steps)) return;
            event.preventDefault();
            setScrub((current) => Math.max(0, Math.min(Math.ceil(state.duration * 10) / 10, (current ?? state.position) + steps[event.key])));
          }}
